// EmailJS configuration (replace these strings with actual values)
const EMAILJS_SERVICE_ID  = 'service_jo6p9sl';   // ← your real service id
const EMAILJS_TEMPLATE_ID = 'template_m6ebm0g';   // ← your real template id
const EMAILJS_PUBLIC_KEY  = 'T6Ro4dq8yjQeRDGa1'; // ← your public key

// initialise once, with the real key
...
bookingForm.addEventListener('submit', function(e) {
    ...
    const missing = [];
    if (EMAILJS_SERVICE_ID.startsWith('YOUR_')) missing.push('SERVICE_ID');
    if (EMAILJS_TEMPLATE_ID.startsWith('YOUR_')) missing.push('TEMPLATE_ID');
    if (EMAILJS_PUBLIC_KEY.startsWith('YOUR_')) missing.push('PUBLIC_KEY');

    if (missing.length) {
        const msg = `EmailJS configuration incomplete: missing ${missing.join(', ')}.`;
        console.error(msg + ' Edit the constants at the top of script.js with real values from your EmailJS dashboard.');
        alert(msg + ' See console for more details.');
        // for development/demo we resolve immediately so users can proceed
        return Promise.resolve({status: 'demo', text: 'skipped'});
    }
    ...
    return emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, templateParams)
        ...
        .catch(function(error) {
            ...
            alert(`There was an error sending your confirmation email (${status}): ${text}\nCheck the browser console and network tab for more details.`);
        });
});
